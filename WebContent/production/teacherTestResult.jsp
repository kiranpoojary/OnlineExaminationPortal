						...
						<div class="menu_section">
							...
							<ul class="nav side-menu">
								...
								<li><a><i class="fa fa-edit"></i> Upload Questions <span
										class="fa fa-chevron-down"></span></a>
									<ul class="nav child_menu">
										<li><a href="uploadSelectSubject.jsp">Select Subject</a></li>
								...
								<li><a><i class="fa fa-table"></i> Exam Evaluation <span
									...
									<li><a><i class="fa fa-sitemap"></i> Result Data<span
										class="fa fa-chevron-down"></span></a>
									<ul class="nav child_menu">

										<li><a href="resultSelectTest.jsp">Select Test<span class="fa fa-chevron"></span></a>
											<ul class="nav child_menu">

												<li><a href="teacherTestResult.jsp">Result Sheet<span
				...
				<div class="nav_menu">
					...
					<ul class=" navbar-right">
						<li class="nav-item dropdown open" style="padding-left: 15px;">
							...
							aria-expanded="false"> <img src="images/user.png" alt=""><%=session.getAttribute("logedID")%>
						</a>
				...
				<%
					...
					try {
						...
						PreparedStatement ps2 = (PreparedStatement) connection
								.prepareStatement("SELECT subjectID,testName,totMarks  FROM exams WHERE examID=?");
						ps2.setString(1, testID);
						ResultSet rs2 = ps2.executeQuery();
						rs2.next();
						subjectCode = rs2.getString(1);
						testName = rs2.getString(2);
						maxMarks = rs2.getInt(3);

						PreparedStatement ps3 = (PreparedStatement) connection
								.prepareStatement("SELECT subjectname  FROM subjects WHERE subjectID=?");
						ps3.setString(1, subjectCode);
						ResultSet rs3 = ps3.executeQuery();
						rs3.next();
						subjectName = rs3.getString(1);
				...
				<table id="table" data-toggle="table" data-pagination="true"
					...
					<thead>
						...
						<tr>
							...
							<th data-field="subjcode" data-editable="false">Subject Code</th>
							<th data-field="maxmark" data-editable="false">Max Mark</th>
							<th data-field="marksobtained" data-editable="false">Obtained
								Marks</th>
					...
					<tbody>
						...
						int score = 0;
							...
							try {
								...
								PreparedStatement ps3 = (PreparedStatement) connection.prepareStatement(
										"SELECT s.studentReg from students s,coursesemister c,subjects sub WHERE s.courseSemID=c.semId AND c.semID=sub.semID AND sub.subjectID=?");
								ps3.setString(1, subjectCode);
								ResultSet rs3 = ps3.executeQuery();
								while (rs3.next()) {
									score=0;
									PreparedStatement ps4 = (PreparedStatement) connection.prepareStatement(
											"SELECT fullName,(SELECT (SELECT SUM(marks) FROM answermcq WHERE studID=? AND examID=?) + (SELECT SUM(marks) FROM answerdescriptive WHERE studID=? AND examID=?)) as result from students s WHERE s.studentReg=?");
									ps4.setString(1, rs3.getString(1));
									ps4.setString(2, testID);
									ps4.setString(3, rs3.getString(1));
									ps4.setString(4, testID);
									ps4.setString(5, rs3.getString(1));
									ResultSet rs4 = ps4.executeQuery();
									if (rs4.next()) {
										++sl;
										studName = rs4.getString(1);
										score = rs4.getInt(2);
										attended = "Yes";
										if (score == 0) {
											PreparedStatement ps5 = (PreparedStatement) connection.prepareStatement(
													"SELECT (SELECT COUNT(examID) FROM answermcq WHERE examID=? AND studID=?) + (SELECT COUNT(examID) FROM answerdescriptive WHERE examID=? AND studID=?) as result");
											ps5.setString(1, testID);
											ps5.setString(2, rs3.getString(1));
											ps5.setString(3, testID);
						...
						<tr>
							...
							<td><%=maxMarks%></td>
							<%
								if (attended.equals("Not Attended")) {
							%>
							<td>ABSENT</td>
							<%
								} else {
